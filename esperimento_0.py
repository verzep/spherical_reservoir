...
for nu in nu_range:
    for tau in tau_range:
        ...
        mse, best_SR, best_scal = simulate_error(x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test,
                                                 neuron_activation=0,  # 0: tanh, 5: spherical, 7: linear
                                                 SR_param=SR_param,
        ...
        np.save('E_test_0_part', np.array(all_error))
        np.save('SR_0_part', np.array(all_SR))
        np.save('scal_0_part', np.array(all_scal))
